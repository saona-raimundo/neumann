...
impl MatrixGame {
    pub fn new<T>(matrix: Array2<T>) -> Self
where
		T: Into<f64>,
    {
        ...
        MatrixGame { matrix: converted_matrix }
    }
    ...
    pub fn row_solve(&self) -> (Array1<f64>, f64) {
        // Define LP
        ...
        for column_action in 0..dimensions[1] {
            ...
            let mut constrain = row_strategy.clone().into_iter().zip(rewards.into_iter().cloned()).collect::<Vec<(minilp::Variable, f64)>>();
        ...
        (Array1::from(optimal_row_strategy), value)
    }
    ...
    pub fn solve(&self) -> (Array1<f64>, Array1<f64>, f64) {
        let column_matrix: Array2<f64> = self.matrix().t().map(|x| -x).to_owned();
        let column_matrix_game = MatrixGame::new(column_matrix);
...
mod tests {
    ...
    #[test_case( array![[0, 1], [1, 0]],  array![0.5, 0.5] ; "positive value")]
    #[test_case( array![[0, 1, -1], [-1, 0, 1], [1, -1, 0]],  array![1./3., 1./3., 1./3.] ; "rock-paper-scisors")]
    fn row_solving<T>(matrix: Array2<T>, expected_strategy: Array1<f64>) 
    where
        T: Into<f64>,
    {
        let matrix_game = MatrixGame::new(matrix);
    ...
    #[test_case( array![[0, 1], [1, 0]],  (array![0.5, 0.5], array![0.5, 0.5], 0.5) ; "positive value")]
    #[test_case( array![[0, 1], [1, 0], [-1, -1]],  (array![0.5, 0.5, 0.], array![0.5, 0.5], 0.5) ; "positive value with extra strategy")]
    #[test_case( array![[0, 1, -1], [-1, 0, 1], [1, -1, 0]],  (array![1./3., 1./3., 1./3.], array![1./3., 1./3., 1./3.], 0.0) ; "rock-paper-scisors")]
    fn solving<T>(matrix: Array2<T>, expected_solution: (Array1<f64>, Array1<f64>, f64)) 
    where
        T: Into<f64>,
    {
        let matrix_game = MatrixGame::new(matrix);
        ...
        for i in 0..expected_solution.0.len() {
            assert_ulps_eq!(optimal_row_strategy[i], expected_solution.0[i], max_ulps = 1);          
        }
        for j in 0..expected_solution.1.len() {
            assert_ulps_eq!(optimal_column_strategy[j], expected_solution.1[j], max_ulps = 1);        
        }
