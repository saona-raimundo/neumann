...
impl PolyMatrixGame {
    ...
    pub fn is_uniform_value_positive(&self) -> bool {
    	if self.degree() == 1 {
    		...
    		unimplemented!("Only implemented for polynomial matrix games which are linear.");
    	}
    }
    ...
    fn linear_is_uniform_value_positive(&self) -> bool {
        ...
        for column_action in 0..dimensions[1] {
            ...
            problem.add_constraint(constrain, minilp::ComparisonOp::Eq, error_free_value);
        }
	...
	// fn old_is_uniform_value_positive(&self) -> bool {
        // // Computing epsilon_0
        // let m: f64 = *self.poly_matrix[0].shape().iter().max().unwrap() as f64;
        // let b: f64 = self
        //     .poly_matrix
...
mod tests {
    ...
    #[test_case( vec![ array![[0]], array![[0]], array![[0]] ], true => panics "Only implemented for polynomial matrix games which are linear.")]
